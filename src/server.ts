...
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
    ...
    // NEW: Matches custom directives -> "label defw expression"
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // --- NEW: 1. Process Custom Data Directives (defw / defb) ---
        const defMatch = line.match(defDirectiveRegex);
        if (defMatch) {
            const potentialLabel = defMatch[1];
            const directive = defMatch[2]; // 'defw' or 'defb'
            const expression = defMatch[3]; // The math expression

            // A. Register the label on the left
            if (labels.has(potentialLabel)) {
                diagnostics.push({
                    severity: DiagnosticSeverity.Error,
                    range: {
                        start: { line: i, character: lines[i].indexOf(potentialLabel) },
                        end: { line: i, character: lines[i].indexOf(potentialLabel) + potentialLabel.length }
                    },
            ...
            // B. Extract all variables/labels from the math expression on the right
            // Capture Groups: 1=Strings, 2=Chars, 3=Hex, 4=Decimals, 5=Labels
            // By matching literals first, we prevent "0x0" from being accidentally parsed as "x0"
            ...
            while ((tokenMatch = expTokenRegex.exec(expression)) !== null) {
                // If group 5 is defined, it means we found a label. 
                // Groups 1-4 (strings, chars, numbers) are safely ignored!
                if (tokenMatch[5]) {
                    const token = tokenMatch[5];
                    const tokenStartIdx = expressionStartIdx + tokenMatch.index;
                    
                    // Prevent standard hardware registers from being flagged as undefined labels
            ...
            continue; // Skip normal instruction parsing for this line
        }
        ...
        if (instMatch && instMatch[1]) {
            ...
            const labelReferencingMnemonics = [
                'jal', 'j', 'beq', 'bne', 'blt', 'bge', 'bltu', 'bgeu', 
                'call', 'beqz', 'bnez', 'tail', 'la'
            ];
