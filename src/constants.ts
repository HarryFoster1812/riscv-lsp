// src/constants.ts
// Added the 'i' flag and simplified the character classes
export const labelRegex = /^\s*([a-z_.][a-z0-9_.]*)(?:\s*:|\s*(?=[#;]|$))/i;
export const instRegex = /^\s*([a-z0-9.]+)(?:\s+([^#;]+))?/i; 
export const defDirectiveRegex = /^\s*([a-z_.][a-z0-9_.]*)\s+(defw|defb|defs|equ)\s+([^#;]+)/i;
export const includeRegex = /^\s*include\s+([^\s#;]+)/i;
export const structRegex = /^\s*(?:[a-z_.][a-z0-9_.]*\s*:?\s+)?struct\b/i;
export const structMemberRegex = /^\s*([a-z_.][a-z0-9_.]*)\s*:?\s+(word|alias)\b(?:\s+(\d+))?/i;
